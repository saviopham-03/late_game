...
* Removed the `System.IO.Hashing.dll` and `System.Runtime.CompilerServices.Unsafe.dll` precompiled assemblies from the package. This avoids compile errors that could occur if those DLLs were included multiple times in a project.
...
* Updated BinarySearch methods "returns" documentation and updated some related unit tests
* Updated the `com.unity.burst` dependency to version `1.8.29`

### Fixed

* Fixed integer overflow crash in container
* Fixed crash in xxhash3 when running on 32 bit Android IL2CPP
